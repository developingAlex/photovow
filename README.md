...
1. Took an hour and a half to get splash screen backdrop image to display off heroku, followed the heroku guides and a couple different stackoverflow answers but what finally fixed it was renaming my application.scss file to application.scss.erb and then using the asset path url helper:<br> `background-image: url('<%= asset_path("image.jpg")%>');`<br>The stackoverflow solutions tried were:
    * [Rails app fails on Heroku: (No route matches [GET] “/”)](https://stackoverflow.com/questions/16862855/rails-app-fails-on-heroku-no-route-matches-get)
    * [No route matches [GET] /assets](https://stackoverflow.com/questions/7829480/no-route-matches-get-assets)
1. Took almost two hours trying to debug why my devise log out links weren't working on heroku. Mysteriously they worked the whole time on my local dev server. The issue was that the links work by making a DELETE request, but in heroku they were being resolved to a GET request. After a lot of online reading came to the conclusion that the issue was with my jquery plugins which apparently are necessary for the DELETE method to work as its non-standard. Ended up on the github page for jquery-ujs and found some instructions I'd never ran before so ran them in an attempt to fix the issue. Read someone elses experience that the ordering of their //=require lines in their application.js file mattered and after they reordered them in a particular way it started working. I checked a classmates application.js and made mine mirror theirs and then it finally worked. Apart from the reordering, making my file match my classmates involved removing two '//= require' lines (jquery and jquery_ujs).<br>the problematic content:
    ```
    //= require jquery3
    //= require turbolinks
    //= require bootstrap
    ...
    //= require_tree .
    ```
    * lesson learned: be careful with the application.js file and know everything about anything you're adding to it.
